...
class SimResults(MahaMulticsConfigFile):
    ...
    .. important::
        ...
        This class does **NOT** provide identical functionality as the Maha
        Multics software.  There are aspects of the Maha Multics software that
        can be unintuitive, so some simplifications have been made to this
        class to make it more intuitive.

        Note that **the numerical simulation results data parsed by this class
        should always match that outputted by the Maha Multics software -- this
        class does NOT alter simulation results data generated by Maha Multics**.
        However, there may be some simulation results files with complex and
        unusual variable names that the Maha Multics software may accept but
        will throw exceptions or be ignored if attempting to read them with
        the :py:class:`SimResults` class.

        For more information, refer to the :py:meth:`SimResults.parse` method.
    """
        ...
        while i < len(self.contents):
            ...
            if (
                ...
                while not re.match(self.__printdict_end_regex, line):
                    # Identify "special" simulation results variable keys
                    ...
                    if line_groups is not None:
                        ...
                        if '*' in asterisk_key:
                            ...
                            key_regex = re.escape(key_regex).replace(
                                r'\*', r'[\*\d]+')
    ...
    def parse(self) -> None:
        ...
        ``printDict`` section can contain any non-whitespace characters.
        One challenge is handling "special" simulation results variables that
        the Maha Multics software uses to match a series of numbered results.
        For instance, a variable named ``pDC_**`` in the ``printDict`` section
        is interpreted by the Maha Multics software to match output simulation
        data such as ``pDC_1``, ``pDC_2``, etc. (i.e., displacement chamber
        pressures for multiple pistons).  In other words, a single key in the
        ``printDict`` section may correspond to multiple simulation results
        variables that need to be added to :py:attr:`SimResults.variables`.

        The convention adopted by the :py:class:`SimResults` class to handle
        this case is as follows:

        - If simulation results variables end with either ``_**`` or ``_*(#,#)``,
          where ``#`` represents an integer, then these are assumed to be
          "special" variables which match numbered results variables.
        - In the event, such a variable is defined in the ``printDict`` section,
          the line in the simulation results file beginning with ``$`` (which
          specifies names and descriptions for simulation results variables) is
          parsed, and the "special" variables in the ``printDict`` section are
          matched against this line.

          - Any "special" variables for which the ``**`` or ``*(#,#)`` portion
            matches some combination of numbers and/or asterisks are added to
            :py:attr:`SimResults.variables`.
          - Otherwise, if no matches are found, the "special" variable is
            ignored (it is not added to :py:attr:`SimResults.variables`).

        Notice that, as discussed in the :py:class:`SimResults` introduction,
        there are some edge cases with unusual variable names that MahaUtils
        will not parse but may be accepted by the Maha Multics software.

        .. dropdown:: "Special" Variable Matching Example
            :animate: fade-in
            ...
            .. code-block:: text
                ...
                $pDC_1:bar:DC1 pressure$pDC_3:bar:DC3 pressure

            After parsing this file, :py:attr:`SimResults.variables` will
            contain ``pDC_1`` and ``pDC_3`` because the "special" variable
            ``pDC_**`` matches these numbered simulation outputs (found on the
            line beginning with ``$``).  However, this line contains no
            entries that match ``qDC_**``, so ``qDC_**`` is ignored.
        """
